...
void GraphicSceneEvent::mouseMoveEvent(QGraphicsSceneMouseEvent *event){
    if(event->buttons() & Qt::LeftButton){
        ...
        if(indexSelectedKnot > 0){
            storageObj.curveChangeEndPosition(indexSelectedKnot - 1, x, y);
        }

        storageObj.knotChange(indexSelectedKnot, x, y);
...
void GraphicSceneEvent::keyPressEvent(QKeyEvent *event){
    if(event->key() == Qt::Key_Backspace){
        storageObj.popBackText(indexSelectedKnot);
        textBackFlag = true;
    }
...
void GraphicSceneEvent::selectedKnot(int x, int y){
    ...
    for(int i = 0; i < 200; ++i){
        posX = storageObj.knotStartPosition_X(i);
        posY = storageObj.knotStartPosition_Y(i);

        if(posX < x && (posX + storageObj.knotWidth(i)) > x && posY < y && (posY + storageObj.knotHeight(i)) > y){
            flag = true;
